...
public class SecurityMgr {
     ...
     * 用来存储登录人员的权限信息
     */
    private Map<String,List<Flyweight>> maps=new HashMap<>();
    ...
    public boolean hasPermit(String user,String securityEntity,String permit){
        // 根据用户名获取保存在缓存中的用户权限
        List<Flyweight> models = maps.get(user);
        ...
        for (Flyweight model : models) {
            ...
            AuthorizationFlyweight flyweight = (AuthorizationFlyweight) model;
            System.out.println("model === " +flyweight.getSecurityEntity()+"  "+flyweight.getPermit());
